...
namespace EasyCharacterMovement
{
    ...
    /// </summary>
    /// 约束在某个平面上，ConstrainXAxis就是X方向不能动，只能在YZ平面上移动
    ...
    public struct FindGroundResult
    {
        ...
        /// <summary>
        /// 通过胶囊投影的结果设置一些地面参数，重载了hitResult里面的point，normal，distance
        /// </summary>
        ...
        /// <param name="sweepDistance"></param>
    ...
    public struct CollisionResult
    {
        ...
        /// The impact point in world space.
        /// </summary>
        /// 世界坐标下角色与碰撞的碰撞点，这个碰撞点是射线检测打到的物体的point
        public Vector3 point;

        /// <summary>
        /// The impact normal in world space.
        /// </summary>
        ///世界坐标下的碰撞法线，这个法线是射线检测打到的物体的normal
        ...
        /// </summary>
        /// 到达碰撞后还剩余的距离 = 要测量的距离 - 实际的距离,比如打10m，实际距离碰撞5m，那么剩余就是5m
        ...
        /// The Rigidbody of the collider that was hit. If the collider is not attached to a rigidbody then it is null.
        /// </summary>
        ///返回碰撞的attachRig，attachRig就是从自己开始递归往上查自己的父物体有没有rig，一般来说不会出现rig嵌套
        public Rigidbody rigidbody => collider ? collider.attachedRigidbody : null;
    ...
    public sealed class CharacterMovement : MonoBehaviour
    {
        ...
        public void InitPara()
        {
            ...
            {
                //capsuleCollider = this.transform.AddComponent<CapsuleCollider>();
                //Debug.LogError(this.name + "未配置CapsuleCollider");
            ...
            if (rigidbody != null)
            {
                ...
                //Debug.LogError(this.name + "未配置Rigidbidy");
            ...
            if (this.name.ToLower().Contains("role"))
                radius = 0.3f;
            ...
            _advanced.maxMovementIterations = 5;
            _advanced.maxDepenetrationIterations = 2;
            _advanced.impartPlatformMovement = true;

            collisionLayers= 1<<0 | 1<<6 | 1<<17  | 1<<18;
        ...
        private enum DepenetrationBehavior
        {
            ...
            IgnoreStatic = 1 << 0,
            /// <summary>
            /// 忽略没有勾选isKinematic的有attachRig的碰撞
            /// </summary>
            IgnoreDynamic = 1 << 1,
            /// <summary>
            /// 忽略勾了isKinematic的有attachRig的碰撞
            /// </summary>
            IgnoreKinematic = 1 << 2
        }
        ...
        {
            /// <summary>
            /// 最小移动距离，如果角色试图移动的距离小于这个距离，它就不会移动。这可以用来减少抖动。在大多数情况下，这个值应该保持为0
            /// </summary>
            ...
            /// <summary>
            /// 启用后计算角色与运动的Rig交互带来的相互作用力，一般不启用，所以是不是可以去掉？
            /// </summary>
            ///[Space(15f)]
            ...
            /// <summary>
            /// 启用后计算角色与角色交互带来的相互作用力，一般不启用，所以是不是可以去掉？
            /// </summary>
            ...
            /// <summary>
            /// 有->无，有->有，无->有，应用平台速度，没看出来表现上有什么差别
            /// </summary>
        ...
        [Tooltip("平台跳跃辅助最大范围    ")]
        [SerializeField]
        private float _jumpAssistOffset;
        ...
        [SerializeField]
        private float _ladderOffset = 1.5f;
        /// <summary>
        /// 角色进入梯子时，以梯子的高度 - ladderOffset 作为角色能到达的最高位置
        /// </summary>
        public float ladderOffset { get { return _ladderOffset; } }
        [SerializeField]
        private float _climbOffset = 1.5f;
        /// <summary>
        /// 角色进入吸附墙时，以吸附墙的高度 - climbOffset 作为角色能到达的最高位置
        /// </summary>
        public float climbOffset { get { return _climbOffset; } }

        /// <summary>
        /// 如果启用，带有SlopeLimitBehavior组件的碰撞器将能够覆盖这个斜率限制
        /// </summary>
        [Space(15f)]
        ...
        private readonly Comparison<RaycastHit> _comparison = Compare;

        public static int Compare(RaycastHit x, RaycastHit y)
        {
            if (x.distance < y.distance)
                return -1;
            else if (x.distance > y.distance)
        ...
        /// 第二次判断重叠的碰撞
        /// </summary>
        private readonly Collider[] _collisionOverlap = new Collider[kMaxOverlapCount];
        ...
        /// 扩展位移，用于计算碰撞回弹
        /// </summary>
		public Vector3 extraDisplacement { get; set; }
        ...
        /// </summary>
        /// 以这个offset来判断周围的碰撞，也用这个判断能不能站到某个碰撞上面，比如一个台阶这样,越过障碍的能力
        ...
        public float jumpAssistOffset
        {
            get => _jumpAssistOffset;
            set => _jumpAssistOffset = Mathf.Max(0.0f, value);
        }
        ...
        /// Note that we still enforce stepOffset to start the step up, this just allows the Character to hang off the edge or step slightly higher off the ground.
        /// </summary>
        /// 允许角色停留在表面的边缘，如果碰撞的Point点位置比offset小，说明角色能站在边缘
        ...
        /// </summary>
        /// 这里是用来判断当我们离开一个碰撞边缘要掉下去的时候再次计算是否能perch时的额外高度
        /// 比如2个台阶，相差的高度为0.5m，如果离开高的那个往下进行射线检测打0.4m，发现没打到，此时角色就会直接掉落下去
        /// 但是有这个perchAdditionalHeight == 0.5m参数实际的检测高度就是0.9m，就能打到矮的台阶，角色会以idle膜下去而不是fall下去
        ...
        /// </summary>
        /// 当启用时，将处理头部碰撞时将会找到surfaceNormal，不太明白
        ...
        public Action<bool> detectCollisionsAction;
        /// <summary>
        ...
        public bool detectCollisions
        {
            ...
            set
            {
                ...
                // 因为我们的collider 同时作为触发器的trigger使用，所以不能关闭
                if (_capsuleCollider)
                    _capsuleCollider.enabled = _detectCollisions;
                //角色碰撞触发改变事件
                detectCollisionsAction.Invoke(value);
            }
        }
        ...
        /// </summary>
        /// 我觉得需要使用
        ///private bool fastPlatformMove { get; set; }
        ...
        /// </summary>
        /// 第一次进入一个平台或者从一个平台切换到另外一个平台时需要应用平台的速度
        // public bool impartPlatformVelocity
        // {
        ...
        /// </summary>
        /// 勾了这个，就能计算角色就能与运动的Rig交互带来的相互作用力
        // public bool enablePhysicsInteraction
        // {
        ...
        /// </summary>
        /// 当进入刚体时(由于质量和相对速度)施加在刚体上的力按此量缩放
		...
		/// <summary>
        /// 在空中运动时，尝试向下移动，抓取到地面就摸上去
        /// </summary>
		public bool tryDownMove{ get; set; }

        /// <summary>
        /// 某些技能不允许离开地面。比如攻击不能出平台边缘
        /// </summary>
        public bool cannotOffCliff { get; set; }

        #endregion

        #region CALLBACKS 碰撞的一些回调
		...
		/// <summary>
		/// Lets you modify the reaction when overlapping with objects.
		/// </summary>
		/// <param name="recover">displacement used to recover</param>
		/// <param name="overlappedCollider">The overlapped collider</param>

		public delegate void CollisionOverlapCallback(Collider[] overlaps, int overlapCount, int depenetrationIterations);
		...
		public CollisionOverlapCallback collisionOverlapCallback { get; set; }
		#endregion
        ...
        #region GEOM_NOMRAL_METHODS 算几何法线的方法
        /// <summary>
        /// 寻找反向法线，待定
        /// </summary>
        /// <param name="sweepDirDenorm"></param>
        /// <param name="inHit"></param>
        /// <returns></returns>
        ...
        private static bool CanJumpAssist(CollisionBehavior behaviorFlags)
        {
            return (behaviorFlags & CollisionBehavior.CanJumpAssist) != 0;
        }

        private static bool CanNotJumpAssist(CollisionBehavior behaviorFlags)
        {
            return (behaviorFlags & CollisionBehavior.CanNotJumpAssist) != 0;
        }
        ...
        public bool ShouldFilter(Collider otherCollider)
        {
            ...
            Collider hitCollider, Transform hitTransform, out Vector3 mtdDirection, out float mtdDistance)
        {
            ...
            if (mtdResult)
            {
                //这里应该是recoverDirection的X,Y,Z都不是Infinity或者NaN
        ...
        private void ResolveOverlaps(DepenetrationBehavior depenetrationBehavior = DepenetrationBehavior.IgnoreNone)
        {
            ...
            for (int i = 0; i < _advanced.maxDepenetrationIterations; i++)
            {
				...
				collisionOverlapCallback?.Invoke(_overlaps, overlapCount, i);
                TryResolveCollisionOverlap(_overlaps, overlapCount, i);
...
#if UNITY_EDITOR
                Debug.DrawLine(position, updatedPosition, Color.red, 0.1f);
#endif
            }
        }
            ...
            out RaycastHit hitResult, out bool startPenetrating, out Vector3 recoverDirection, out float recoverDistance, bool ignoreNonBlockingOverlaps = false)
        {
            ...
            _hits.Sort(0, rawHitCount, _comparison);
        ...
        /// Returns True when the rigidbody sweep intersects any collider, otherwise false.
        /// </summary>
        /// 沿着位移向量扫描角色的体积，如果检测到碰撞，则在命中点附近停止，如果未检测到碰撞，则应用全部位移。
        private bool MovementSweepTest(Vector3 characterPosition, Vector3 inVelocity, Vector3 displacement,
            out CollisionResult collisionResult,Vector3 dir = default)
        {
            ...
            {
                //方向为朝向的单位向量，距离算向量的长度
                sweepDirection = dir.normalized;
                sweepDistance = dir.magnitude;
            }
            else
            {
        ...
        private Vector3 ComputeSlideVector(Vector3 displacement, Vector3 inNormal, bool isWalkable, CollisionResult inHit)
        {
            if (isGrounded)
            {
                if (isWalkable)
                ...
                {
                    //如果地面上碰到怪物的边缘不需要计算边缘滑行
                    if (computeSliderVectorCallBack.Invoke(inHit))
                    {
            ...
            ref Vector3 displacement, ref CollisionResult inHit, ref Vector3 prevNormal)
        {
            ...
            {
                inVelocity = ComputeSlideVector(inVelocity, inHit.normal, true, inHit);
                displacement = ComputeSlideVector(displacement, inHit.normal, true, inHit);
            }
            else
            {
                if (iteration == 0)
                {
                    inVelocity = ComputeSlideVector(inVelocity, inHit.normal, inHit.isWalkable, inHit);
                    displacement = ComputeSlideVector(displacement, inHit.normal, inHit.isWalkable, inHit);
                ...
                else if (iteration == 1)
                {
                    ...
                    Vector3 nVel = ComputeSlideVector(displacement, inHit.normal, inHit.isWalkable, inHit);
        ...
        private void PerformMovement(float deltaTime,Vector3 dir = default)
        {
            // Resolve initial overlaps

            DepenetrationBehavior depenetrationFlags = DepenetrationBehavior.IgnoreDynamic;
            ...
            //这一帧的位移
            Vector3 displacement = _velocity * deltaTime + extraDisplacement;
			
            extraDisplacement = Vector3.zero;
            ...
            //吸附墙的辅助攀爬，区别正常的辅助攀爬，只考虑角色的朝向，不考虑速度
            if (dir != Vector3.zero)
            {
                int adsorbMaxSlideCount = _advanced.maxMovementIterations;
                while (detectCollisions && adsorbMaxSlideCount-- > 0)
                {
                    //如果是吸附的话就往朝向的方向的向量扫描碰撞
                    bool collided = MovementSweepTest(updatedPosition,_velocity, displacement,
                        out CollisionResult collisionResult, dir);
                    ...
                    if (!isConstrainedToGround && !isOnWalkableGround)
                    {
                        CheckLanded(ref collisionResult);

                        if (!_hasLanded && collisionResult.hitLocation != HitLocation.Below)
                        {
                            if(CanClimb(collisionResult.collider) && Climb(ref collisionResult, out float y, deltaTime,dir,0.3f))
                            {
                                updatedPosition = new Vector3(updatedPosition.x, y, updatedPosition.z);
                                displacement = Vector3.zero;
                                climbUpCallBack?.Invoke(collisionResult.collider);
                            }
                        }
                    }
            ...
            else
            {
                ...
                while (detectCollisions && maxSlideCount-- > 0 && displacement.sqrMagnitude > _advanced.minMoveDistanceSqr)
                {
                    ...
                    if (!collisionResult.isWalkable)
                    {
                        if (CanJumpAssist(collisionResult.collider) &&
                            JumpAssist(ref collisionResult, out CollisionResult sweepPastResult))
                        {
                            updatedPosition = sweepPastResult.position;
                    ...
                    if (isConstrainedToGround && !isOnWalkableGround)
                    {
                        ...
                        if (!_hasLanded)//去掉这个判断是因为，有些形状可能是微微的梯形，确实下半部分碰到了角色 && collisionResult.hitLocation != HitLocation.Below)
                        {
                            if (CanClimb(collisionResult.collider) && Climb(ref collisionResult, out float y,deltaTime))
                            {
                                updatedPosition = new Vector3(updatedPosition.x, y, updatedPosition.z);
                                displacement = Vector3.zero;
                                climbUpCallBack?.Invoke(collisionResult.collider);
			...
			// 还在空中,并且需要膜到地面去，那再往下打一个距离
			if (tryDownMove && !isGrounded && !_hasLanded)
			{
                bool downSweep = MovementSweepTest(updatedPosition, Vector3.zero, Vector3.down * stepOffset, out var downSweepResult);
				if(downSweep)
				{
                    updatedPosition += downSweepResult.displacementToHit;
                    CheckLanded(ref downSweepResult);
				}
            }
        ...
        void CheckLanded(ref CollisionResult collisionResult)
        {
            //falling的时候检查碰撞是不是能站人
            if (IsValidLandingSpot(updatedPosition, ref collisionResult))
            {
                _hasLanded = true;
                landedVelocity = collisionResult.velocity;
            ...
            {
                // See if we can convert a normally invalid landing spot (based on the hit result) to a usable one.
                //如果不能站人，尝试在无效的着陆点碰撞后也就是基于hit的结果找一个有效的着陆点
        ...
        private bool GroundSweepTest(Vector3 characterPosition, float capsuleRadius, float capsuleHalfHeight,
            float sweepDistance, out RaycastHit hitResult, out bool startPenetrating, LayerMask layerMask)
        {
            ...
            if (!useFlatBaseForGroundChecks)
            {
                ...
                foundBlockingHit = CapsuleCast(point1, point2, capsuleRadius, sweepDirection, sweepDistance,
                    layerMask , out hitResult, out startPenetrating);
            }
            else
            {
                ...
                LayerMask sweepLayerMask = layerMask;
        ...
        public void ComputeGroundDistance(Vector3 characterPosition, float sweepRadius, float sweepDistance,
            float castDistance, out FindGroundResult outGroundResult, LayerMask groundDistanceLayerMask = default)
        {
            ...
            LayerMask layerMask = groundDistanceLayerMask == default ? _collisionLayers : groundDistanceLayerMask;
            // Sweep test
            ...
            if (sweepDistance > 0.0f && sweepRadius > 0.0f)
            {
                ...
                foundGround = GroundSweepTest(characterPosition, capsuleRadius, capsuleHalfHeight, actualSweepDistance,
                    out RaycastHit hitResult, out startPenetrating, layerMask);
                ...
                if (foundGround || startPenetrating)
                {
                    ...
                    if (startPenetrating || !IsWithinEdgeTolerance(characterPosition, hitResult.point, capsuleRadius))
                    {
                        ...
                        foundGround = GroundSweepTest(characterPosition, capsuleRadius, capsuleHalfHeight,
                            actualSweepDistance, out hitResult, out startPenetrating, layerMask);
                    }
            ...
            if (castDistance > 0.0f)
            {
                ...
                foundGround = Raycast(rayOrigin, rayDirection, rayLength, layerMask, out RaycastHit hitResult);
        ...
        public void FindGround(Vector3 characterPosition, out FindGroundResult outGroundResult)
        {
            ...
            if (outGroundResult.hitGround && !outGroundResult.isRaycastResult)
            {
                ...
                {
                    //当角色碰到的Collider的Point点已经不在Perch Offset范围内了，角色要掉下去了，这里需要再算下ComputePerchResult
        ...
        private bool CanJumpAssist(Collider otherCollider)
        {
            // Validate input collider
            ...
            if (collisionBehaviorCallback != null)
            {
                ...
                if (CanJumpAssist(collisionBehavior))
                    return true;

                if (CanNotJumpAssist(collisionBehavior))
                    return false;
            }
        ...
        private bool JumpAssist(ref CollisionResult inCollision, out CollisionResult sweepResult)
        {
            sweepResult = default;
            ...
            float stepTravelUpHeight = Mathf.Max(0.0f, jumpAssistOffset - actualGroundDistance);
            float stepTravelDownHeight = jumpAssistOffset + kMaxGroundDistance * 2.0f;
            ...
            if (deltaY > 0.0f && !CanJumpAssist(hitResult.collider))
                return false;

            // Output new position on step.

            sweepResult = new CollisionResult
            {
                position = positionOnStep
            };
        ...
        /// <summary>
        /// 换人时取消移动平台，需要当帧就取消，不能等到换人出来跑到物理系统更新
        /// </summary>
        public void SetPlatformAsNull()
        {
        ...
        //     //对方的速度比我大，那么characterImpulse就是我的被推挤的速度将通过质量比加上，对方也要被推挤
        ...
        public CollisionFlags Move(Vector3 newVelocity, float deltaTime = 0.0f,Vector3 dir = default)
        {
            // if (deltaTime == 0.0f)
            ...
            PerformMovement(deltaTime, dir);


            //插件原代码
            //if (isGrounded || _hasLanded)
            //  FindGround(updatedPosition, out testGround);
            //我需要一个功能，如果技能会移动出平台，那么不允许移动出去。典型的就是在平台边缘攻击。
            FindGroundResult testGround = default;
            if (isGrounded || _hasLanded)
            {
                FindGround(updatedPosition, out testGround);
                ...
                if (cannotOffCliff)
                {
                    if (testGround.isWalkableGround)
                    {
                        _foundGround = testGround;
                    }
                    else
                    {
                        //再检测一次原先位置的地面， 防止地面移动或消失导致角色悬空
                        FindGround(position, out testGround);
                        if (testGround.hitGround)
                        {
        ...
        public CollisionFlags Move(float deltaTime = 0.0f,Vector3 dir = default)
        {
            return Move(_velocity, deltaTime, dir);
        }
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (stepOffset > 0.0f)
            {
                DrawDisc(footPosition + transform.up * stepOffset, rotation, radius * 1.15f,
                    new Color(0.969f, 0.357f, 0.545f, 0.75f), false);
            }
        }
        ...
        public UnityAction<Collider> climbUpCallBack { get; set; }

        public Func<CollisionResult, bool> computeSliderVectorCallBack { get; set; }

        private static bool CanClimb(CollisionBehavior behaviorFlags)
        {
            return (behaviorFlags & CollisionBehavior.CanClimb) != 0;
        }

        private static bool CanNotClimb(CollisionBehavior behaviorFlags)
        {
            return (behaviorFlags & CollisionBehavior.CanNotClimb) != 0;
        }
        /// <summary>
        /// 通过回调判断物体能不能爬上去
        /// </summary>
        ...
        /// <returns></returns>
        private bool CanClimb(Collider otherCollider)
        {
            if (otherCollider == null)
                return false;
            ...
            if (collisionBehaviorCallback != null)
            {
                ...
                if (CanClimb(collisionBehavior))
                    return true;

                if (CanNotClimb(collisionBehavior))
                    return false;
            }
            return false;
        }

        private bool Climb(ref CollisionResult inCollision, out float y, float deltaTime, Vector3 dir = default,float offset = 0.12f)
        {
			// 不应过大，避免撞到天花板
            float stepTravelUpHeight = 1.6f;
            y = 0;
            var delta = Vector3.zero;
			// Step up, treat as vertical wall
			
			Vector3 sweepOrigin = inCollision.position;
            if(inCollision.rigidbody != null)
            {
				delta = inCollision.rigidbody.velocity *= deltaTime;
				sweepOrigin.y += delta.y > 0 ? delta.y : 0;
			}
			Vector3 sweepDirection = _characterUp;
            ...
            //打自己半径的2倍算位置
            // Step forward (lateral displacement only)
            sweepDistance = _radius * 2;
            //在这里设置一下朝向，用于吸附时用辅助攀爬
            if (dir == Vector3.zero)
            {
                sweepDirection = Vector3.ProjectOnPlane(velocity, _characterUp).normalized;
            }
            else
            {
                sweepDirection = dir.normalized;
            ...
            sweepDirection = -_characterUp;
            sweepDistance = stepTravelUpHeight;
            ...
            float deltaY = hitResult.point.y - inCollision.position.y;
            y = hitResult.point.y - climbOffset;
            ...
            return Mathf.Abs(deltaY - climbOffset) < offset;
        }
        Vector3 computePosition = Vector3.zero;
        private int collisionOverlapCount;
        
        /// <summary>
        /// 在第一次碰撞检测之后预计算位置，防止穿墙
        /// </summary>
        /// <param name="overlaps"></param>
        /// <param name="overlapCount"></param>
        /// <param name="depenetrationIterations"></param>
        public void TryResolveCollisionOverlap(Collider[] overlaps, int overlapCount, int depenetrationIterations)
        {
            computePosition = updatedPosition;
            for (int i = 0; i < overlapCount; i++)
            {
                Collider overlappedCollider = overlaps[i];
                if (ShouldFilter(overlappedCollider))
                    continue;

                if (ComputeMTD(computePosition, updatedRotation, overlappedCollider, overlappedCollider.transform,
                        out Vector3 recoverDirection, out float recoverDistance)) 
                {
                    ...
                    computePosition += impactNormal * (recoverDistance + kPenetrationOffset);
                    
                }
            }
            Vector3 top = computePosition + _transformedCapsuleTopCenter;
            Vector3 bottom = computePosition + _transformedCapsuleBottomCenter;
            int collisionOverlapCount = Physics.OverlapCapsuleNonAlloc(bottom, top, _radius, _collisionOverlap, _collisionLayers, triggerInteraction);
            collisionOverlapCallback(_collisionOverlap, collisionOverlapCount, depenetrationIterations);
        }
    }
}